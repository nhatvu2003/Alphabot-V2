...
const code = require("../helpers/code.cjs");
...
module.exports = {
  //logs - Enhanced for AlphaBot UI
  log(...args) {
    const message = args.join(' ');
    if (message.includes('Logging in')) {
      console.log(`🔑 ${code.cyan('[LOGIN]')} Đang tiến hành đăng nhập...`);
    } else if (message.includes('Logged in!')) {
      console.log(`✅ ${code.green('[LOGIN]')} Đăng nhập thành công!`);
    } else if (message.includes('Choosing the best region')) {
      console.log(`🌍 ${code.blue('[LOGIN]')} Đang chọn region tối ưu...`);
    } else if (message.includes('Connected to specified region')) {
      console.log(`⚡ ${code.cyan('[LOGIN]')} Đã kết nối đến region`);
    } else if (message.includes('Successfully logged in')) {
      console.log(`🔒 ${code.green('[LOGIN]')} Appstate được bảo mật - không gửi về bên thứ 3`);
    } else if (message.includes('Hello')) {
      // Extract name and ID from "Hello, Nhat Vu (100006462752972)"
      const match = message.match(/Hello,\s*(.+?)\s*\((\d+)\)/);
      if (match) {
        const name = match[1];
        const id = match[2];
        console.log(`✅ ${code.green('[LOGIN]')} Đăng nhập thành công với ID: ${id}`);
      }
    }
    // Skip other logs to keep UI clean
  },
  error(...args) {
    // Print error messages only to avoid exposing file paths and stacks in logs
    const safeArg = (a) => {
    ...
    // mark Error objects as already-logged to avoid duplicate printing elsewhere
    args.forEach(a => {
      try {
        if (a instanceof Error) a.__alphabot_logged = true;
      } catch (e) { /* ignore */ }
    });

    const processed = args.map(safeArg);
    console.error(`❌ ${code.red('[ERROR]')}`, ...processed);
  },
  warn(...args) {
    const message = args.join(' ');
    if (message.includes('No region is specified')) {
      // Skip this warning as it's not critical
      return;
    }
    console.warn(`⚠️ ${code.yellow('[WARNING]')}`, ...args);
  },
  //end logs
  isReadableStream,
  cleanGet,
