...
async function checkAppstate(APPSTATE_PATH, APPSTATE_PROTECTION) {
    ...
    const { isTermux, isGlitch } = global.modules.get("environments.get");
...
async function getAppStateNoProtection(
    ...
    try {
        if (isJSON(appState)) {
            ...
            // For Termux, get key from environment or use default
            APPSTATE_SECRET_KEY = process.env.APPSTATE_SECRET_KEY || global.config?.GBOTWAR_CONFIG?.APPSTATE_SECRET_KEY || 'termux-default-key';
...
async function getAppStateWithProtection(APPSTATE_PATH, appState, isTermux) {
    try {
        let objAppState, APPSTATE_SECRET_KEY;
        const logger = global.modules.get("console");
        const aes = global.modules.get("aes");
        ...
        if (isTermux) {
            // For Termux, use environment variable or generate new key
            APPSTATE_SECRET_KEY = process.env.APPSTATE_SECRET_KEY || global.config?.GBOTWAR_CONFIG?.APPSTATE_SECRET_KEY;
            
            if (!APPSTATE_SECRET_KEY) {
                APPSTATE_SECRET_KEY = getRandomPassword();
                // Save the key to config for future use
                if (global.config && global.config.GBOTWAR_CONFIG) {
                    global.config.GBOTWAR_CONFIG.APPSTATE_SECRET_KEY = APPSTATE_SECRET_KEY;
                    global.config.save();
                }
            }
