...
async function loadCommands() {
  ...
  let total = 0;
  const commandsPath = resolvePath(process.cwd(), "src", "commands");
...
async function loadOnMessage() {
  ...
  console.log("[DEBUG] onMessagePath:", onMessagePath);
...
async function loadEvents() {
  const logger = global.modules.get("console");
  const eventsPath = resolvePath(process.cwd(), "src", "events");
